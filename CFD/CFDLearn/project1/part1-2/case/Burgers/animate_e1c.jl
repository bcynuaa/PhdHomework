...
save_path::String = "..//..//data//Burgers//explicit_bc1_conservation//";
...
using Plots;
gr();
image_path::String = "..//..//image//Burgers//explicit_bc1_conservation//"
...
for j = 1:length(c)
    for i = 1:length(func_name)
        println("nu=$(c_name[j]), $(func_name[i]) IC");
        ...
        @time anim = @animate for k = 1:skip:size(data)[1]
            ...
            plot(
                ...
                label="\$ \\frac{\\partial u}{\\partial t} + u\\frac{\\partial u}{\\partial x} = $(c[j])\\frac{\\partial^2 u}{\\partial x^2} \$"
            );
            xlabel!("\$ x \$")
            ylabel!("\$ u \$")
        end
